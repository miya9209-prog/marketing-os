...
if col_btn[0].button("초기화"):
    st.session_state.clear()
    st.rerun()

save_name = f"misharp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
col_btn[1].download_button("작업 저장", save_state(), file_name=save_name)

uploaded = col_btn[2].file_uploader("작업 불러오기", type=["json"])
if uploaded:
    data = json.load(uploaded)
    st.session_state.update(data)
    st.success("불러오기 완료")

col_btn[3].link_button("이미지추출", "https://misharp-image-crop-v1.streamlit.app/")
col_btn[4].link_button("URL 단축", "https://shor.kr")
...
if st.button("문구 생성"):
    ...
    if not selected:
        ...
        if "push" in selected and not out.startswith("광고)"):
            out = "광고)" + out
        if "kakao" in selected:
            out += f"\n상품 바로가기 ▼\n{st.session_state.get('url','')}"
        st.text_area("결과", out, height=400)
